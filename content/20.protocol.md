...
* Crystallographic Binary File CBS/imgCIF [@doi:10.1107/97809553602060000729] - instrument-specific metadata. HBF
...
5. Extract the [link relations](https://tools.ietf.org/html/rfc8288) [@doi:10.17487/RFC8288] of type [alternate](https://html.spec.whatwg.org/multipage/links.html#rel-alternate) to find the download links
...
* To list filename contained in a ZIP file, the whole file must be downloaded because in the [ZIP format](https://pkware.cachefly.net/webdocs/casestudies/APPNOTE.TXT) the table of content is written at the end of the file
  * The host providing Zenodo downloads do not support the [HTTP Range](https://tools.ietf.org/html/rfc7233) request header [@doi:10.17487/RFC7233], so it is not possible to download say only the last 128 kB of the ZIP file.
...
* `application/x-datacite-v41+xml` giving DataCite v4 XML [@doi:10.5438/0012] 
...
* Join the JSON files using the ASCII Record Separator (RS, `0x1e`)
  to make a `application/json-seq` JSON text sequence [@doi:10.17487/RFC7464] stream
