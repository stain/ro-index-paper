...
<small><em>
This manuscript
([permalink](https://stain.github.io/ro-index-paper/v/a6e68b6112ee62c0095d79d637bbeb735c941c45/))
was automatically generated
from [stain/ro-index-paper@a6e68b6](https://github.com/stain/ro-index-paper/tree/a6e68b6112ee62c0095d79d637bbeb735c941c45)
on December 3, 2019.
</em></small>
...
5. Extract the [link relations](https://tools.ietf.org/html/rfc8288) [@ozdWB3O7] of type [alternate](https://html.spec.whatwg.org/multipage/links.html#rel-alternate) to find the download links
...
* To list filename contained in a ZIP file, the whole file must be downloaded because in the [ZIP format](https://pkware.cachefly.net/webdocs/casestudies/APPNOTE.TXT) the table of content is written at the end of the file
  * The host providing Zenodo downloads do not support the [HTTP Range](https://tools.ietf.org/html/rfc7233) request header [@8nlwFIw8], so it is not possible to download say only the last 128 kB of the ZIP file.
...
In order to estimate the bandwidth and storage requirements for
executing the above prototype workflow across the whole of Zenodo,
a shell script approach was used to retrieve and analyse the JSON
for each Zenodo record, which include information on downloadable 
filename, extension and file size.

To retrieve these it was deemed necessary to use 
the [undocumented](https://github.com/zenodo/zenodo/issues/1426) parts of [Zenodo API](https://developers.zenodo.org/).
...
workflow. Below is a bash transcript. Execution time was about 3 days
from a server at the University of Manchester network on a single 1 GBps
network link.  The script does:

* Retrieve each of the first 3.5 million Zenodo records  
  as Zenodo JSON
  by iterating over possible numeric IDs
  (current maximum ID `3450000` estimated from ["Recent uploads"](https://zenodo.org/))
...
Downloading all will take at least 4 days assuming 73.9 MBit/s
as measured using `wget` of a [9 GB file](https://zenodo.org/api/files/2cdaea21-80be-48b9-9ab2-259671795f7a/br20832_cores.zip) - 
